...
build_tidy_dataset <- function(type, df_actName, dfcolNames){
  ...
  colnames(d_features) <- dfcolNames
...
d_activity <- join(d_act_lbl_id, df_actName)
...
merge_datasets <- function(){
  ...
  tidy_test <- build_tidy_dataset("test", d_act_lbl_name, v_column_names )
  
  setwd(mypath)
  #setwd("C:/Manonita Mishra/Data Science Projects/~John Hopkins/Getting and Cleaning Data/Week4/UCI HAR Dataset")
  tidy_train <- build_tidy_dataset("train", d_act_lbl_name, v_column_names)
...
mean_std_df <- function(df_final){

  # 2. Extracts only the measurements on the mean and standard deviation for each measurement
  
  final_dataset_col_names <- names(df_final)
  mean_std_columns_data <- df_final[,grep("mean|std",final_dataset_col_names)]
  mean_std_columns_data
}

clean_df_columnnames <- function(df_final){

  # 4. Appropriately labels the data set with descriptive variable names.
  # Replace strings:
  tempColNames <- names(df_final)
  # t -> Time
  tempColNames <- sub("^t", "Time", tempColNames)
  # f -> Frequency
  tempColNames <- sub("^f", "Frequency", tempColNames)
  # Gyro -> Gyroscope
  ...
  names(df_final) <- tempColNames
  df_final
}

average_df <- function(df_final){
  # 5. From the data set in step 4, creates a second, 
  # independent tidy data set with the average of each variable for each activity and each subject.
  
  d_tidy_average <- aggregate(df_final, by = list(df_final$SubjectId, df_final$ActivityId), FUN = "mean")
}


##########################
generateCodeBook <- function(df_final) {
  # Generate the code book
  library(dataMaid)
  library(rmarkdown)
  
  makeCodebook(df_final, reportTitle = "Codebook for Tidy data", file = "CodeBook.Rmd")

}

### SAMPLE CODE
#set the working directory to the location of the unzipped file

mydf <- merge_datasets()
my_mean_std_df <- mean_std_df(mydf)
mytidydf <- clean_df_columnnames(mydf)
my_average_df <- average_df(mytidydf)
